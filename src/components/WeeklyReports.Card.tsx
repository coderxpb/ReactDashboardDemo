  ...
  return (
    <RoundedCard maxWidth={tabletSize ? "120px" : "200px"}>
      <Stack sx={{alignItems: "center", marginTop: `${tabletSize ? '14px' : '28px'}`, marginBottom: `${tabletSize ? '14px' : '28px'}`}}>
        ...
        <Typography variant={"body2"} sx={{fontSize: `${tabletSize ? '12px' : '20px'}`, color: colors.primary}}>
          {title}
        </Typography>
        <Box sx={{height: "14px"}}/>

        {selected != value ?
          <Typography variant={"h6"} sx={{color: colors.primary, fontWeight: 600, margin: "-3px", fontSize: `${tabletSize ? '' : '26px'}`}}>
            {storage} Gb
          </Typography>
          :
